...
class Cluster_Manager_albedo:
    ...
    def load(self, cluster_config_file):
        ...
        for i, config in enumerate(configs):
            if config is None:
                ...
                cluster = Cluster_albedo(cluster_dir=dir)
    ...
    def update_center(self, labels, pixels, quantile=0.3, n_samples=5000, band_factor=0.5):
        ...
        self.clusters = []
        if self.class_num == 1:
            cluster = Cluster_albedo()
            cluster.update_center(pixels.reshape(-1, 3), quantile=quantile, n_samples=n_samples,
